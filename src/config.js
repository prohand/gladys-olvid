...
export const DEFAULT_CONFIG = {
  // gRPC endpoint of the Olvid daemon (the "Olvid application" the integration
  // drives). http:// -> plaintext, https:// -> TLS.
  daemon_url: 'http://olvid-daemon:50051',
  // Admin client key of the daemon (OLVID_ADMIN_CLIENT_KEY_* env variable of
  // the daemon container). Secret: no default, filled in by the user.
  admin_client_key: '',
...
export const INTERNAL_CONFIG_KEYS = ['client_key'];
...
export function normalizeConfig(raw = {}) {
  return {
    ...
    // Internal storage: the identity client key minted by the integration.
...
export function requiresReconnect(previous, next) {
  return ['daemon_url', 'admin_client_key', 'identity_id'].some(
    (key) => previous[key] !== next[key],
  );
}

/**
 * Tell whether the configuration holds the minimum needed to connect.
 * @param {Record<string, unknown>} config - Normalized configuration.
 * @returns {boolean} True when the daemon URL and the admin key are set.
 * @example
